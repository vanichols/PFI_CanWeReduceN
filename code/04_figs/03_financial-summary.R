...
d_money_fig %>% 
  ...
  labs(x = NULL,
       ...
       title = str_wrap("Majority of trials saw potential for savings when reducing N rates",
                        width = 80),
       subtitle = "Five trials saw financial losses in all price scenarios, the remaining 12 saw potential savings") + 
  my_money_theme


# fig by unit N  different range style-------------------------------------------------------------
d_money_fig %>% 
  left_join(d_diffs %>% select(last_name, dif_nrate_lbac) %>% distinct()) %>% 
  filter(last_name == "Bardole")
